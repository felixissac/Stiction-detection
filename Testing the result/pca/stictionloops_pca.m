function correct_prediction = stictionloops_pca(trainednet)
    ...
    if i == 1     %%BUILDING
     ...
     if i == 3   %MINING 
        ...
        for j=1:size(loops_min,1)
            OP_value = ((cdata.(process(i)).("loop"+loops_min(j)).OP(1:500,:)));
            PV_value = ((cdata.(process(i)).("loop"+loops_min(j)).PV(1:500,:)));
            newinput = pvalues_center(OP_value,PV_value);
            output = sim(trainednet, newinput);
            result(count,1) = output(1);
            result(count,2) = output(2);
